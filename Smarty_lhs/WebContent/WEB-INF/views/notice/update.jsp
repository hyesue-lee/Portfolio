...
//			loadScript('http://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.9/summernote.js', function() {
		...
		$('#btnUpdate').on('click', function(){ // 온클릭시 굳이 function doWrite() 펑션 선언,호출하지 말고 바로 종속시켜 기능 먹인다. 
...
function deleteFile(fileIdx, boardSeq){
	  ...
	  if("${sessionScope.memberId}" != null) {
	        	...
	        	$.ajax({
	        		url: "<c:url value='/notice/deleteAttFile.do' />",
	        		...
	        		success: function(result, textStatus, XMLHttpRequest){
	        			...
	        			if(result.result == 1){
	        				...
	        				movePage(this, "/notice/goToUpdate.do?boardSeq="+boardSeq);
	        			}else{
	        				alert(result.msg);
	        				return;
	        			}
...
<body>
		...
		<c:if test="${empty attFiles}"> <!-- 첨붚파일없으면  -->
