...
$(document).ready(function(){
	...
	$('#btnUpdate').on('click', function(){
		...
		var formData = new FormData(frm);
	     $.ajax({
	         url : "<c:url value='/board/goToUpdate.do'/>",
	         data : formData,
	         type : 'POST',
	         dataType : "text",
	         processData : false,
			...
			var formData = new FormData(frm);
		     $.ajax({
		         ...
		         success : function (result, textStatus, XMLHttpRequest) {
		             ...
		             if(data.result == 1){
		                movePage(this, "/board/list.do");
		             } else {
		               window.location.href="javascript:movePage(null,'/board/read.do?boardSeq=${boardInfo.board_seq }')";
...
<input type="hidden" name="typeSeq" value ="${boardInfo.type_seq }"/>
					...
					<a href="javascript:movePage(null, '/board/goToUpdate.do?boardSeq=${boardInfo.board_seq}')">
					...
					<c:choose>
			        		...
			        		<c:otherwise>
			        			<a href="javascript:movePage(null, '/board/list.do?page=${page}')">
						        <button type="button" class="btn btn-primary">목록</button>
						   		</a>
			        		</c:otherwise>
			        </c:choose>
